...
public sealed class CardFormat : IndexableFormat<Card>
{
    ...
    public CardFormat(bool caseInsensitiveParsing, IFormatter<Rank> rankFormat, IFormatter<Suit> suitFormat)
        : this(caseInsensitiveParsing, Card.FullDeck().Select(c => $"{rankFormat.Format(c.Rank)}{suitFormat.Format(c.Suit)}"))
    {
    }

    public CardFormat(bool caseInsensitiveParsing, IFormatter<Rank> rankFormat, IFormatter<Suit> suitFormat, string separator)
        : this(caseInsensitiveParsing, Card.FullDeck().Select(c => $"{rankFormat.Format(c.Rank)}{separator}{suitFormat.Format(c.Suit)}"))
    {
    }

    public CardFormat(bool caseInsensitiveParsing, [JetBrains.Annotations.InstantHandle] IEnumerable<string> values)
        : this(caseInsensitiveParsing, values.ToArray())
    {
    }
    
    public CardFormat(bool caseInsensitiveParsing, params string[] values)
